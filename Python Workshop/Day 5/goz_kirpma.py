...
def calculate_EAR(eye_points, landmarks, image_w, image_h):
    ...
    p3 = np.array([landmarks[eye_points[3]].x * image_w, landmarks[eye_points[3]].y * image_h])
    ...
    vertical = np.linalg.norm(p1 - p4)
    vertical2 = np.linalg.norm(p2 - p3)
    horizontal = np.linalg.norm(p0 - p3b)
    EAR = (vertical + vertical2) / (2.0 * horizontal)
    return EAR
...
while cap.isOpened():
    ...
    image = cv2.flip(image, 1)  # Görüntüyü yatayda çevir (ayna düzeltme)
    ...
    if results.multi_face_landmarks:
        ...
        cv2.putText(frame, f"Blink Count: {blink_toplam}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
